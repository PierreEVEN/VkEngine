//VERSION : Friday, 17 July 2020 09:02:37

/**** GENERATED FILE BY REFLECTION TOOL, DO NOT MODIFY ****/

#undef _REFL_FILE_UNIQUE_ID_
#define _REFL_FILE_UNIQUE_ID_ RUID_0 // File unique ID (used by reflection macros)


#ifndef _REFL_Object_H
#define _REFL_Object_H


#include "Reflection.h"




/* ##############################  Reflection for OObject  ############################## */

class OObject; // Forward declaration

#define _REFLECTION_BODY_RUID_0_LINE_12 REFL_DECLARE_CLASS(OObject)

REFL_DECLARE_TYPENAME(OObject); // Declare typename

template<> struct RIsReflected<OObject> { static constexpr bool Reflect = true; }; // Used to detect if class is reflected or not


#endif
