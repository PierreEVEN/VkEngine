//VERSION : Monday, 24 August 2020 23:11:27

/**** GENERATED FILE BY REFLECTION TOOL, DO NOT MODIFY ****/

#undef _REFL_FILE_UNIQUE_ID_
#define _REFL_FILE_UNIQUE_ID_ RUID_3 // File unique ID (used by reflection macros)


#ifndef _REFL_ShaderModule_H
#define _REFL_ShaderModule_H


#include "Reflection.h"




/* ##############################  Reflection for ShaderModule  ############################## */

namespace Rendering
{
	class ShaderModule; // Forward declaration

	#define _REFLECTION_BODY_RUID_3_LINE_15 REFL_DECLARE_CLASS(ShaderModule)

	REFL_DECLARE_TYPENAME(ShaderModule); // Declare typename

	template<> struct RIsReflected<ShaderModule> { static constexpr bool Reflect = true; }; // Used to detect if class is reflected or not
}


#endif
